...
import PsnMgrService from '../services/psn-mgr-service';
...
export default {
  ...
  effects: {
    * fetch({ payload: { search } }, { call, put }) {
      const tableData = yield call(PsnMgrService.list, search);
    ...
    * getBasicDetailData({ payload: { record } }, { call, put }) {
      if (record.personId && record.personId !== '') {
        const detailData = yield call(PsnMgrService.getBasicDetailData, record.personId);
        yield put({
          type: 'stateWillUpdate',
    ...
    * queryDetailData({ payload: { detailSearch } }, { call, put }) {
      const detailData = yield call(PsnMgrService.getInfoSetDetailData, detailSearch);
      yield put({
        type: 'stateWillUpdate',
    ...
    * getRefData({ payload: { url, search } }, { call, put }) {
      const tableData = yield call(PsnMgrService.getRefData, url, search);
      const formatTable = formatTableData(tableData);
      yield put({
        type: 'stateWillUpdate',
        payload: {
